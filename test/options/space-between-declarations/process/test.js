describe.skip('options/space-between-declarations:', function() {
    describe('process', function() {
        ...
        it('Integer value => should set proper space after declaration', function() {
            ...
            return this.shouldBeEqual('integer-value.css', 'integer-value.expected.css');
        });

        it('Valid string value (spaces only) => should set proper space after declaration', function() {
            this.comb.configure({ 'space-between-declarations': ' ' });
            return this.shouldBeEqual('space-value.css', 'space-value.expected.css');
        });

        it('Valid string value (spaces and newlines) => should set proper space after declaration', function() {
            this.comb.configure({ 'space-between-declarations': '\n    ' });
            return this.shouldBeEqual('space-newline-value.css', 'space-newline-value.expected.css');
        });

        it('Should leave comments as is', function() {
            this.comb.configure({ 'space-between-declarations': 1 });
            return this.shouldBeEqual('comments.css', 'comments.expected.css');
        });

        it('Issue 239', function() {
            this.comb.configure({ 'space-between-declarations': '\n    ' });
            return this.shouldBeEqual('issue-239.css', 'issue-239.expected.css');
        });

        it('Issue 378', function() {
            this.comb.configure({ 'space-between-declarations': '\n' });
            return this.shouldBeEqual('issue-378.css');
        });
    });
});
