...
function cssToAST(text, syntax, filename) {
    ...
    try {
        tree = gonzales.srcToAST({ syntax: syntax, src: text, needInfo: true });
    } catch (e) {
...
function detectInNode(node, level, handler, detectedOptions) {
    node.forEach(function(node) {
        ...
        if (Array.isArray(detected) && Array.isArray(detected[0])) {
            variants.push.apply(variants, detected);
        } else if (typeof detected !== 'undefined' && detected.length) {
 ...
 * @returns {Array}
 */
function lintDetectedOptions(detected, handlers, config) {
    var violations = {};
    var configOptions = Object.keys(config);
    Object.keys(detected).forEach(function(option) {
        var values = detected[option];
        if (configOptions.indexOf(option) !== -1 && values.length) {
            violations[option] = values.filter(function(item) {
                return item[0] !== config[option];
            });
        }
...
/**
 * Lint the options in the given file
 *
 * @param {String} path Path to the stylesheet
 * @param {Array} config List of options to detect
 * @returns {Object} Detected options
 */
CSScomb.lintFile = function lintFile(path, config) {
    var stylesheet = fs.readFileSync(path, 'utf8');
    return CSScomb.lintString(stylesheet, config);
};

/**
 * Lint the options in the given string
 *
 * @param {String} text Stylesheet
 * @param {Array} config Config
 * @returns {Object} Detected options
 */
CSScomb.lintString = function lintString(text, config) {
    var result;
    var handlers = [];
    ...
    var tree = cssToAST(text);
    var detectedOptions = detectInTree(tree, handlers);
    result = lintDetectedOptions(detectedOptions, handlers, config);
