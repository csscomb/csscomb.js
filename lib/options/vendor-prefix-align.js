    ...
    return {
        ...
        detect: function(nodeType, node) {
            ...
            var getResult = function(node, sum, info, i) {
                ...
                if (node.length === i + 4 && partialResult !== null) {
            ...
            walk({
                ...
                getExtraSymbols: extraIndentProperty,
                payload: function(info, i) {
                    if (node[i - 1] && typeof node[i - 1][1] !== 'object') {
            ...
            walk({
                ...
                getExtraSymbols: extraIndentVal,
                payload: function(info, i) {
                    for (var x = node[i].length - 2; x >= 0; x--) {
                        if (node[i][x][0] === 'value') break;
                    }
            ...
            if (result.true > 0 || result.false > 0) {
                if (result.true >= result.false) {
                    return [true, node[node.length - 1]];
                } else {
                    return [false, node[node.length - 1]];
                }
            }
        }
    };
