...
if (program.lint) {
    ...
    var result = Comb.lintFile(file, config);
    var messages = [];
    Object.keys(result).forEach(function(rule) {
        messages = messages.concat(result[rule].map(function(item) {
            return 'Rule {rule} violation at {file} {line}:{column}\n'
                .replace('{rule}', rule)
                .replace('{file}', file)
                .replace('{line}', item[1].ln)
                .replace('{column}', item[1].col);
        }));
    });
