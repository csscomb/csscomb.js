...
program
    ...
    .option('-i, --interactive', 'interactive mode (allows fix files after linting)')
...
if (fs.existsSync(configPath)) {
    ...
    config.interactive = program.interactive;
    ...
    var processFiles = function() {
        ...
        .then(function(changedFiles) {
            ...
            if (config.interactive && changedFiles.length !== changed) {
                var prompt = function() {
                    inquirer.prompt([{
                        type: "list",
                        name: "actions",
                        ...
                        if (answers.actions === 'all') {
                            config.lint = false;
                            config.interactive = false;
                            processFiles();
                        } else if (answers.actions === 'file') {
                            inquirer.prompt([{
                                name: 'file',
                                type: 'checkbox',
                                message: 'Choose file',
                                choices: program.args
                            }], function(answers) {
                                console.log(answers)
                                if (answers.file.length > 0) {
                                    config.lint = false;
                                    config.interactive = false;
                                    program.args = [answers.file];
                                    processFiles();
                                } else {
                                    prompt();
                                }
            ...
            if (config.lint && !config.interactive && tbchanged) {
                process.exit(1);
            }
